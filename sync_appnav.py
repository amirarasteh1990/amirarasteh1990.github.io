...
TABS = [
    ...
    # Reverted, as the note here always said it would be: a second collection has
    # shipped, so /paintings/ is a choice between galleries rather than a page whose
    # only content is one door. The tab lands on the index again. current_for()
    # matches on the paintings/ prefix, so it stays lit inside every gallery too.
