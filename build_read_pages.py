...
def patch_status_page(check: bool) -> bool:
    ...
    if STATUS_PAGE.is_file() and STATUS_PAGE.read_text(encoding="utf-8") == page:
        ...
        return True
    if check:
        print("[drift] /sedaha/languages/: page missing or stale")
        return False
    STATUS_PAGE.parent.mkdir(parents=True, exist_ok=True)
    STATUS_PAGE.write_text(page, encoding="utf-8", newline="\n")
    tally = ", ".join(f"{sum(1 for r in rows if r['state'] == s)} {lbl.lower()}" for s, lbl, _ in STATES)
    print(f"[write] sedaha/languages/index.html  ({tally})")
    return True
...
def patch_sitemap(check: bool) -> bool:
    ...
    if "/sedaha/languages/</loc>" not in body:
        if check:
            print("[drift] sitemap: /sedaha/languages/ missing")
            return False
